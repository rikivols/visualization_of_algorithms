    ...
    def find_path(self, start: tuple[int], end: tuple[int]) -> bool:
        ...
        while self.heap:
            ...
            for next_direction in self.directions:
                ...
                new_distance = distance + 1 + self.calculate_heuristic(next_node, end)
                if next_node not in self.distances or new_distance < self.distances[next_node]:
                    self.distances[next_node] = new_distance
